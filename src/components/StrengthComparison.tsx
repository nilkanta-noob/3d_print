      ...
      {MATERIALS.map((material) => {
        ...
        return (
          <div key={material.slug} className="grid grid-cols-[4.5rem_1fr] items-center gap-x-5 gap-y-2 sm:grid-cols-[5.5rem_1fr_5rem]">
            <span className="font-display text-xl font-medium tracking-[-0.03em] text-text-primary">{material.name}</span>
            <div className="h-1.5 overflow-hidden bg-text-primary/10" role="img" aria-label={`${material.name} strength: ${label}, ${level} of 5`}>
              <div className="h-full bg-accent-primary" style={{ width: `${level * 20}%` }} />
            </div>
